    ...
    def save_some_images(self, n: int = 10):
        ...
        with torch.no_grad():
            for idx in range(0, len(dataset), len(dataset) // n):
                x, y, _ = dataset[idx]  # (3, 256, 256), (256, 256)
                y = y.float()

                pred = self.clf(torch.unsqueeze(x, 0).to(self.device))  # (1, 2, 256, 256)
                _, pred_cls = torch.max(pred[0], dim=0)
                pred_cls = pred_cls.float().cpu()
                # pred_sm = torch.softmax(pred, dim=1)[0, 0].cpu()

                save_image(x, base / f"ds-{idx}-x.png")
                save_image(y, base / f"ds-{idx}-y.png")
                # save_image(pred_sm, base / f"ds-{idx}-pred.png")
