...
class fixmatch(Strategy):
    ...
    def _train(self, epoch, loader_tr_labeled, loader_tr_unlabeled, optimizer):
        ...
        for batch_idx, (x, y, idxs) in enumerate(loader_tr_labeled):
            ...
            ## Segmentation loss
            ## TODO should we add an activation or something before the loss? i've put softmaxes (not the case in the original code)
        ...
        #return accFinal / (len(loader_tr_labeled.dataset.X)*loader_tr_labeled.dataset.X.shape[1]*loader_tr_labeled.dataset.X.shape[2]), train_loss
