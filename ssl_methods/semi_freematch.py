...
from strategy_utils_framework.utils import time_string, AverageMeter, RecorderMeter, convert_secs2time, adjust_learning_rate, dice_loss, dice_score
...
from preprocess.transformations import ColorHEDAugmentation
from strategy_utils_framework.utils import EarlyStopping
...
class freematch:
    ...
    def __init__(self, X, Y, X_val, Y_val, idxs_lb, net, handler, args,n_pool,device,predict,g):
    ...
    def _train(self, epoch, loader_tr_labeled, loader_tr_unlabeled, optimizer, glob_thresh, class_p,onehot_class_p):
        ...
        for batch_idx, (x, y, idxs) in enumerate(loader_tr_labeled):
            ...
            output_u, output_u2 = torch.chunk(output_unsup, 2) #torch.Size([N, 2, 64, 64])
            ...
            if iter_unlabeled != None:
                ...
                max_probs, max_idx = torch.max(pseudo_label, dim=1) #qb=max prob of each pixel, ~qb=pseudo label of each pixel 
                sample_max_probs = torch.mean(max_probs, (1,2),keepdim=True) #adaptation for segmentation, torch.Size([N,1,1])
            ...
            # TODO: I suppose this is multiclass (we use 1 number per class) and not binary (would use 1 number for the positive class and infer neg.)
    ...
    def train(self, alpha=0.1, n_epoch=10, batch_ratio=0.6, callback=True,n_iter=0):
        ...
        early_stopping = EarlyStopping(patience=20,path=checkpoint_path,delta=0.03,verbose=True)
        min_val_loss = 99999
        ...
        if idxs_unlabeled.shape[0] != 0:
            ...
            loader_tr_unlabeled = DataLoader(train_data_unlabeled,
                                             ...
                                             worker_init_fn=self.seed_worker,
                                             generator=self.g,
                                             **self.args.loader_tr_args)#int(2250 * batch_ratio)
