...
def inf_empty_ds(channel: int = 3, height: int = 256, width: int = 256):
    "infinite (Tensor(), Tensor(), None, None) generator"
...
class flexmatch(Strategy):
    ...
    def update_batch(self, idx: torch.Tensor, mask: torch.Tensor, max_idx: torch.Tensor):
        ...
        """
        # max_prob > T, get the class
        # count the number of prob>T for each class for each image
        for ib_id, ulb_id in enumerate(idx):
            valid_idxs = max_idx[ib_id][mask[ib_id]]  # [256, 256]
    ...
    def setup_selected_labels(self, ulb_data: DataLoader):
        ...
        # fill it up
        for (u_w, _), _, idx in ulb_data:  # [N, 3, W, H], [N]
            with torch.no_grad():
                output_w = self.clf(u_w.to(self.device))  # [N, 2, 256, 256]
                pseudo_label = torch.softmax(output_w, dim=1)
                max_probs, max_idx = torch.max(pseudo_label, dim=1)  # [N, 256, 256]
    ...
    def _train(self, epoch, loader_tr_labeled, loader_tr_unlabeled, optimizer):
        ...
        for batch_idx, (x, y, idxs) in enumerate(loader_tr_labeled):
            nb_batch = batch_idx
            # inputs_u = weak augment, inputs_u2 = strong augment
            try:
            ...
            output_u, output_u2 = torch.chunk(output_unsup, 2)  # torch.Size([N, 3, 256, 256])
